...
<div>
	...
	<div class="card mb-4">
		<div class="card-header">Add Title Configuration</div>
		<div class="card-body">
			<form method="POST" action="{{ route('configurations.title.store') }}">
				@csrf
				<div class="row g-3">
					<div class="col-md-4">
	...
	<div class="card">
		<div class="card-header">Title Configurations</div>
		<div class="card-body">
			<form method="GET" action="{{ route('configurations.title') }}" class="row g-2 mb-3">
				...
				<div class="col-md-2 d-grid">
					<a href="{{ route('configurations.title') }}" class="btn btn-outline-secondary">Reset</a>
				</div>
			</form>
			<table class="table table-bordered align-middle">
				...
				<tbody>
							...
							<td>
								...
								<div class="modal fade" id="{{ $modalId }}" tabindex="-1" aria-hidden="true">
									<div class="modal-dialog">
											<form method="POST" action="{{ route('configurations.title.update', $title) }}">
												@csrf
												@method('PUT')
												<div class="modal-content">
													<div class="modal-header">
														<h5 class="modal-title">Edit Title Configuration</h5>
					...
					@empty
						<tr>
							<td colspan="3" class="text-center text-muted">No title configurations yet.</td>
						</tr>
					@endforelse
				</tbody>
			</table>
			<div class="d-flex justify-content-between align-items-center">
				<div class="text-muted">Showing {{ $titles->firstItem() ?? 0 }} - {{ $titles->lastItem() ?? 0 }} of {{ $titles->total() }}</div>
				{{ $titles->links() }}
			</div>
		</div>
		</div>
