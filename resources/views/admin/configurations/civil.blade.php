	...
	<div class="card mb-4">
		...
		<div class="card-body">
			<form method="POST" action="{{ route('configurations.civil.store') }}">
				...
				<div class="row g-3">
					<div class="col-md-4">
						<label class="form-label">Civil Code</label>
						<input type="text" name="civil_code" class="form-control @error('civil_code') is-invalid @enderror" value="{{ old('civil_code') }}" required>
						@error('civil_code')
					...
					<div class="col-md-6">
						...
						<input type="text" name="civil_status" class="form-control @error('civil_status') is-invalid @enderror" value="{{ old('civil_status') }}" required>
						@error('civil_status')
							<div class="invalid-feedback">{{ $message }}</div>
						@enderror
					</div>
	...
	<div class="card">
		...
		<div class="card-body">
			<form method="GET" action="{{ route('configurations.civil') }}" class="row g-2 mb-3">
				<div class="col-md-4">
					<input type="text" name="q" class="form-control" value="{{ request('q') }}" placeholder="Search civil code or civil status...">
				</div>
			...
			<table class="table table-bordered align-middle">
				<thead>
					<tr>
						<th style="width: 200px;">Civil Code</th>
						<th>Civil Status</th>
				...
				<tbody>
					@forelse($civils as $civil)
						...
						<tr>
							<td class="align-middle">{{ $civil->civil_code }}</td>
							<td class="align-middle">{{ $civil->civil_status }}</td>
							<td>
											...
											<form method="POST" action="{{ route('configurations.civil.update', $civil) }}">
												...
												<div class="modal-content">
													...
													<div class="modal-body">
														<div class="mb-3">
															<label class="form-label">Civil Code</label>
															<input type="text" name="civil_code" value="{{ $civil->civil_code }}" class="form-control" required>
														</div>
														<div class="mb-2">
															<label class="form-label">Civil Status</label>
															<input type="text" name="civil_status" value="{{ $civil->civil_status }}" class="form-control" required>
														</div>
													</div>
